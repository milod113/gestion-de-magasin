...
@section('title', 'Modifier une catégorie d’équipement')
@section('subtitle', 'Mettre à jour les informations de la famille d’appareils')
...
<div class="max-w-3xl mx-auto">
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-100 dark:border-gray-700 overflow-hidden">
        ...
        <form method="POST" action="{{ route('immobilier.categories-equipements.update', $category) }}" class="p-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="name" value="{{ old('name', $category->name) }}"
                           class="w-full px-3 py-2.5 rounded-lg border @error('name') border-red-500 dark:border-red-500 @else border-gray-300 dark:border-gray-600 @enderror bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                           required>
                    @error('name')
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="text" name="code" value="{{ old('code', $category->code) }}"
                               class="w-full pl-9 pr-3 py-2.5 rounded-lg border @error('code') border-red-500 dark:border-red-500 @else border-gray-300 dark:border-gray-600 @enderror bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
                    </div>
                    @error('code')
                        <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                    @enderror
                    <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                        Code court unique, utilisé dans les exports et rapports.
                    </p>
                </div>
            </div>
            ...
            <div>
                ...
                <textarea name="description" rows="4"
                          ...
                          placeholder="Décrivez les types d’appareils concernés...">{{ old('description', $category->description) }}</textarea>
                @error('description')
                    <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex justify-between items-center pt-4 border-t border-gray-100 dark:border-gray-700">
                <form action="{{ route('immobilier.categories-equipements.destroy', $category) }}" method="POST"
                      onsubmit="return confirm('Supprimer cette catégorie ? Les équipements resteront, mais seront non catégorisés.');">
                    @csrf
                    @method('DELETE')
                    <button type="submit"
